...
function rx_touch (el, elCoords, precise) {   // (SVGElement, SVGElement, [Boolean]) -> observable of observables of { x: Int, y: Int }
  ...
  //
  const touchDragObs = function (touchStart, wantedId) {      // (TouchEvent, ???tbd.???) -> observable of {x:Int, y:Int}
    ...
    return innerObs( el, elCoords, touchStart, moveObs, cancelOrEndObs, precise );

  }; // touchDragObs

  // tbd. move this to above, eliminating 'touchDragObs' (once things work). #refactor
  //
  // The "touchstart" event can start multiple drags. This actually happens on iOS (9.2) but not on Android (6.0.1).
  //
  return startAllObs.flatMap( ev => {  // (TouchEvent) -> Array of observable of {x:Int, y:Int}

    return range( 0, ev.changedTouches.length ).map( i => {   // tbd. iterate array contents directly
        const touchStart = ev.changedTouches[i];

        return touchDragObs( touchStart, i );   // tbd. is 'i' the wanted id?   (compare with old code if now crooked)
    } );
  });
};  // rx_touch
...
SVGElement.prototype.rx_draggable = function (myCoords, precise) {   // ([Boolean] = false, [Boolean]) -> observable of observables of { x: Int, y: Int }
  ...
  return merge(
    ...
    //rx_touch(this, elCoords, precise)   // disabled
  );
};
